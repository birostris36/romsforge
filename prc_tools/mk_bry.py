# -*- coding: utf-8 -*-
"""
Created on Thu Nov 14 17:06:04 2024
...
from utils.ncCreate import create_bry_ust
import xarray as xr
import numpy as np
...
'''
TIME_UNIT=OGCM_TIMES.units
OGCM_times=num2date(OGCM_TIMES[:],TIME_UNIT,calendar='noleap')
...
TIMES_co=np.arange(len(OGCM_TIMES))
...
create_bry_ust(My_Bry,mask,topo,MyVar,NSEW,OGCM_TIMES[:],TIME_UNIT,Bry_title,ncFormat='NETCDF4')
...
if NSEW[0]:
    ...
    # North (gridbuilder version)
    
    bry_lat_co=np.where( (latO[:,0]>=np.min(latG[:,:2])-latOmax) & (latO[:,0]<=np.max(latG[:,:2])+latOmax) )[0]
    bry_lon_co=lonO_co
    ...
    #for i in ['temp']:
        ...
        if (i=='zeta') or (i=='ubar') or (i=='vbar'):
            data=np.zeros([len(Bry_time_num),lonG.shape[-2],2])
            if i=='zeta':
            ...
            tmp_mask_=np.invert(DATA.mask)    
            data=np.zeros([len(Bry_time_num),len(depthO),lonG.shape[-2],2])
    ...
    vbar_north= ru2.rho2u_2d(OGCM_Data['vbar'][:,:,-2]*cosa[:,0,:,-2]-OGCM_Data['ubar'][:,:,-2]*sina[:,0,:,-2])
    
    #Process 3D vectors
    u=ru2.rho2u_4d(OGCM_Data['u'][:,:,:,:2]*cosa[:,:,:,:2]+OGCM_Data['v'][:,:,:,:2]*sina[:,:,:,:2]).squeeze()
    v=ru2.rho2u_3d(OGCM_Data['v'][:,:,:,-2]*cosa[:,:,:,-2]-OGCM_Data['u'][:,:,:,-2]*sina[:,:,:,-2])
    ...
    for i,n in zip(OGCM_Data['zeta'],range(Rzeta.shape[0])):
        ...
        zw[n,:,:,:]=ru.zlevs(MyVar['Vtransform'],MyVar['Vstretching'],MyVar['Theta_s'],\
                 MyVar['Theta_b'],MyVar['Tcline'],MyVar['Layer_N'],\
                     5,topo[:,:2],i) # -2: ???
    zu=ru2.rho2u_4d(zr_).squeeze()
    zv=ru2.rho2v_4d(zr_)[:,:,:,-2]
    dzr=zw[:,1:,:,:]-zw[:,:-1,:,:] # [t,depth,lat,lon]
    dzu=ru2.rho2u_4d(dzr).squeeze();
    dzv=ru2.rho2v_4d(dzr)[:,:,:,-2];
    zr=zr_[:,:,:,-2]
    
    # Add a level on top and bottom with no-gradient
    temp,salt=OGCM_Data['temp'][:,:,:,-2],OGCM_Data['salt'][:,:,:,-2]
...
if NSEW[1]: # Southern bry
    ...
    cosa_,sina_=np.cos(angle)[:,-2:],np.sin(angle)[:,-2:] #SOUTHERN BRY
    cosa=np.tile( np.tile(cosa_,(thO,1,1)), (len(Bry_time_num),1,1,1) )
    sina=np.tile( np.tile(sina_,(thO,1,1)), (len(Bry_time_num),1,1,1) )
    
    #Process 2D vectors
    #modi
    ubar_south= ru2.rho2u_3d(OGCM_Data['ubar'][:,:,-2:]*cosa[:,0,:,-2:]+OGCM_Data['vbar'][:,:,-2:]*sina[:,0,:,-2:]).squeeze()
    vbar_south= ru2.rho2u_2d(OGCM_Data['vbar'][:,:,-1]*cosa[:,0,:,-1]-OGCM_Data['ubar'][:,:,-1]*sina[:,0,:,-1])
    
    #ori
    #ubar_north= ru2.rho2u_2d(OGCM_Data['ubar'][:,-1,:]*cosa[:,0,-1,:]+OGCM_Data['vbar'][:,-1,:]*sina[:,0,-1,:])
    #vbar_north= ru2.rho2v_3d(OGCM_Data['vbar'][:,-2:,:]*cosa[:,0,-2:,:]+OGCM_Data['ubar'][:,-2:,:]*sina[:,0,-2:,:]).squeeze()
    ...
    Rzeta=OGCM_Data['zeta'][:,:,-1] # -1 for southern BRY
    ...
    for i,n in zip(OGCM_Data['zeta'],range(Rzeta.shape[0])):
        ...
        zw[n,:,:,:]=ru.zlevs(MyVar['Vtransform'],MyVar['Vstretching'],MyVar['Theta_s'],\
                 MyVar['Theta_b'],MyVar['Tcline'],MyVar['Layer_N'],\
                     5,topo[:,-2:],i) # -2: ???
    zu=ru2.rho2u_4d(zr_).squeeze()
    zv=ru2.rho2v_4d(zr_)[:,:,:,-1]
    dzr=zw[:,1:,:,:]-zw[:,:-1,:,:] # [t,depth,lat,lon]
    dzu=ru2.rho2u_4d(dzr).squeeze();
    dzv=ru2.rho2v_4d(dzr)[:,:,:,-1];
    zr=zr_[:,:,:,-1]
...
if NSEW[2]: # Eastern bry
    ...
    #Process 2D vectors
    #modi
    ubar_east= ru2.rho2u_2d(OGCM_Data['ubar'][:,-1,:]*cosa[:,0,-1,:]+OGCM_Data['vbar'][:,-1,:]*sina[:,0,-1,:])
    vbar_east= ru2.rho2v_3d(OGCM_Data['vbar'][:,-2:,:]*cosa[:,0,-2:,:]-OGCM_Data['ubar'][:,-2:,:]*sina[:,0,-2:,:]).squeeze()
...
if NSEW[3]: # Western bry
    ...
    #Process 2D vectors
    #modi
    ubar_west= ru2.rho2u_2d(OGCM_Data['ubar'][:,-2,:]*cosa[:,0,-2,:]+OGCM_Data['vbar'][:,-2,:]*sina[:,0,-2,:])
    ...
    #Process 3D vectors
    u=ru2.rho2u_3d(OGCM_Data['u'][:,:,-2,:]*cosa[:,:,-2,:]+OGCM_Data['v'][:,:,-2,:]*sina[:,:,-2,:])
    v=ru2.rho2v_4d(OGCM_Data['v'][:,:,:2,:]*cosa[:,:,:2,:]-OGCM_Data['u'][:,:,:2,:]*sina[:,:,:2,:]).squeeze()
    ...
    Rzeta=OGCM_Data['zeta'][:,-1,:] # -1 for Western BRY
